...
export default function Home() {
  ...
  const fetchPosts = async (reset = false) => {
    ...
    try {
      ...
      const postsData = data.map((post: any) => {
        ...
        let albumImages = (post.album_images || []).map((img: string) =>
          img?.startsWith('/storage/v1/object/public/')
            ? `${process.env.EXPO_PUBLIC_SUPABASE_URL}${img}`
            : img
        );
        ...
        return {
          ...
          type: post.type || (albumImages.length > 0 ? 'album' : 'single')
        };
      });
  ...
  const fetchPromotions = async () => {
    try {
      ...
      setPromotions(promotionsData);
      if (posts.length > 0) combineFeedItems(posts, promotionsData);
    } catch (error) {
      console.error('Error fetching promotions:', error);
    }
  };
