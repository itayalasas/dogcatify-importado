...
export default function Home() {
  ...
  const incrementPromotionViews = async (promotionId: string) => {
    try {
      const { error } = await supabaseClient
        .from('promotions')
        .update({ 
        ...
        {loading ? (
          ...
          feedItems.map((item, index) => {
            if (item.type === 'promotion') {
              return (
                <View key={`promotion-${item.data.id}`}>
                  <PromotionCard
                    promotion={item.data}
                    onPress={() => handlePromotionPress(item.data)}
                  />
                  {/* Increment views when promotion is rendered */}
                  {React.useEffect(() => {
                    incrementPromotionViews(item.data.id);
                  }, [])}
                </View>
              );
            } else {
