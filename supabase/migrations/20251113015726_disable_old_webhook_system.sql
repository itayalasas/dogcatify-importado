...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'ERROR: Trigger order_created_crm_webhook no existe. Aplica primero la migración create_crm_webhook_system.sql';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'ERROR: Trigger order_updated_crm_webhook no existe. Aplica primero la migración create_crm_webhook_system.sql';
  END IF;
